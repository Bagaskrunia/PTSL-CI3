...
class Tengaran extends CI_Controller {

    ///Load model
    public function __construct()
    {
        parent::__construct();
        $this->load->model('tengaran_model');
        ///Proteksi Halaman
	...
	{
        $tengaran = $this->tengaran_model->listing();

        $data = array(  'title'             => 'tengaran',
                        'tengaran'     => $tengaran,
                        'isi'               => 'kecamatan/tengaran/data'
        );
	...
	public function tambah()
	{
        ...
        $valid->set_rules('no_berkas','No Berkas','required|is_unique[tb_tengaran.no_berkas]',
            array(	'required'    => '%s harus diisi',
					'is_unique'   => '%s sudah ada. Buat Nomor Berkas Baru'));

		$valid->set_rules('nop','NOP','required|is_unique[tb_tengaran.nop]',
            array(  'required'      => '%s harus diisi',
                    'is_unique'   => '%s sudah ada. Buat NOP Baru'));
		
		$valid->set_rules('nama','Nama');
			
		$valid->set_rules('nik','NIK','required|is_unique[tb_tengaran.nik]',
            array(  'required'    => '%s harus diisi',
        ...
        if($valid->run()==FALSE)    {
            ...
            $data = array(  'title'     => 'Tambah Data tengaran',
                            'isi'       => 'kecamatan/tengaran/tambah'
            );
            $this->load->view('layout/wrapper', $data, FALSE);
        ///Masuk Database
        }else{
            ...
            $this->tengaran_model->tambah($data);
            $this->session->set_flashdata('sukses', 'Data telah ditambah');
            redirect(base_url('kecamatan/tengaran'),'refresh');
        }
    }
	...
	public function edit($id)
	{
        ...
        $tengaran = $this->tengaran_model->detail($id);
        ...
        if($valid->run()==FALSE)    {
            ...
            $data = array(  'title'              => 'Edit Data tengaran (no berkas = '.$tengaran->no_berkas.' )',
                            'tengaran'      => $tengaran,
                            'isi'                => 'kecamatan/tengaran/edit'
            );
            $this->load->view('layout/wrapper', $data, FALSE);
        ///Masuk Database
        }else{
    ...
    {
        $data = array('id' => $id);
        $this->tengaran_model->delete($data);
	...
	public function export_excel()
	{
        ...
        $data = array(  'title'             => 'Daftar Nominatif PTSL Kecamatan tengaran',
                        'tengaran'     => $tengaran
        );
		$this->load->view('kecamatan/tengaran/laporan_excel', $data, FALSE);
	}
}
