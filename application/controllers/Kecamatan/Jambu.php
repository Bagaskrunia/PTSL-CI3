...
class Jambu extends CI_Controller {

    ///Load model
    public function __construct()
    {
        parent::__construct();
        $this->load->model('jambu_model');
        ///Proteksi Halaman
	...
	{
        $jambu = $this->jambu_model->listing();

        $data = array(  'title'             => 'jambu',
                        'jambu'     => $jambu,
                        'isi'               => 'kecamatan/jambu/data'
        );
	...
	public function tambah()
	{
        ...
        $valid->set_rules('no_berkas','No Berkas','required|is_unique[tb_jambu.no_berkas]',
            array(	'required'    => '%s harus diisi',
					'is_unique'   => '%s sudah ada. Buat Nomor Berkas Baru'));

		$valid->set_rules('nop','NOP','required|is_unique[tb_jambu.nop]',
            array(  'required'      => '%s harus diisi',
                    'is_unique'   => '%s sudah ada. Buat NOP Baru'));
		
		$valid->set_rules('nama','Nama');
			
		$valid->set_rules('nik','NIK','required|is_unique[tb_jambu.nik]',
            array(  'required'    => '%s harus diisi',
        ...
        if($valid->run()==FALSE)    {
            ...
            $data = array(  'title'     => 'Tambah Data jambu',
                            'isi'       => 'kecamatan/jambu/tambah'
            );
            $this->load->view('layout/wrapper', $data, FALSE);
        ///Masuk Database
        }else{
            ...
            $this->jambu_model->tambah($data);
            $this->session->set_flashdata('sukses', 'Data telah ditambah');
            redirect(base_url('kecamatan/jambu'),'refresh');
        }
    }
	...
	public function edit($id)
	{
        ...
        $jambu = $this->jambu_model->detail($id);
        ...
        if($valid->run()==FALSE)    {
            ...
            $data = array(  'title'              => 'Edit Data jambu (no berkas = '.$jambu->no_berkas.' )',
                            'jambu'      => $jambu,
                            'isi'                => 'kecamatan/jambu/edit'
            );
            $this->load->view('layout/wrapper', $data, FALSE);
        ///Masuk Database
        }else{
	...
	public function export_excel()
	{
        ...
        $data = array(  'title'             => 'Daftar Nominatif PTSL Kecamatan jambu',
                        'jambu'     => $jambu
        );
		$this->load->view('kecamatan/jambu/laporan_excel', $data, FALSE);
	}
}
