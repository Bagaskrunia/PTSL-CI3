...
class Tuntang extends CI_Controller {

    ///Load model
    public function __construct()
    {
        parent::__construct();
        $this->load->model('tuntang_model');
        ///Proteksi Halaman
	...
	{
        $tuntang = $this->tuntang_model->listing();

        $data = array(  'title'             => 'Tuntang',
                        'tuntang'     => $tuntang,
                        'isi'               => 'kecamatan/tuntang/data'
        );
	...
	public function tambah()
	{
        ...
        $valid->set_rules('no_berkas','No Berkas','required|is_unique[tb_tuntang.no_berkas]',
            array(	'required'    => '%s harus diisi',
					'is_unique'   => '%s sudah ada. Buat Nomor Berkas Baru'));

		$valid->set_rules('nop','NOP','required|is_unique[tb_tuntang.nop]',
            array(  'required'      => '%s harus diisi',
                    'is_unique'   => '%s sudah ada. Buat NOP Baru'));
		
		$valid->set_rules('nama','Nama');
			
		$valid->set_rules('nik','NIK','required|is_unique[tb_tuntang.nik]',
            array(  'required'    => '%s harus diisi',
        ...
        if($valid->run()==FALSE)    {
            ...
            $data = array(  'title'     => 'Tambah Data Tuntang',
                            'isi'       => 'kecamatan/tuntang/tambah'
            );
            $this->load->view('layout/wrapper', $data, FALSE);
        ///Masuk Database
        }else{
            ...
            $this->tuntang_model->tambah($data);
            $this->session->set_flashdata('sukses', 'Data telah ditambah');
            redirect(base_url('kecamatan/tuntang'),'refresh');
        }
    }
	...
	public function edit($id)
	{
        ...
        $tuntang = $this->tuntang_model->detail($id);
        ...
        if($valid->run()==FALSE)    {
            ...
            $data = array(  'title'              => 'Edit Data Tuntang (no berkas = '.$tuntang->no_berkas.' )',
                            'tuntang'      => $tuntang,
                            'isi'                => 'kecamatan/tuntang/edit'
            );
            $this->load->view('layout/wrapper', $data, FALSE);
        ///Masuk Database
        }else{
    ...
    {
        $data = array('id' => $id);
        $this->tuntang_model->delete($data);
	...
	public function export_excel()
	{
        ...
        $data = array(  'title'             => 'Daftar Nominatif PTSL Kecamatan Tuntang',
                        'tuntang'     => $tuntang
        );
		$this->load->view('kecamatan/tuntang/laporan_excel', $data, FALSE);
	}
}
