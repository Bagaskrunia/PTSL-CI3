...
class Suruh extends CI_Controller {

    ///Load model
    public function __construct()
    {
        parent::__construct();
        $this->load->model('suruh_model');
        ///Proteksi Halaman
	...
	{
        $suruh = $this->suruh_model->listing();

        $data = array(  'title'             => 'suruh',
                        'suruh'     => $suruh,
                        'isi'               => 'kecamatan/suruh/data'
        );
	...
	public function tambah()
	{
        ...
        $valid->set_rules('no_berkas','No Berkas','required|is_unique[tb_suruh.no_berkas]',
            array(	'required'    => '%s harus diisi',
					'is_unique'   => '%s sudah ada. Buat Nomor Berkas Baru'));

		$valid->set_rules('nop','NOP','required|is_unique[tb_suruh.nop]',
            array(  'required'      => '%s harus diisi',
                    'is_unique'   => '%s sudah ada. Buat NOP Baru'));
		
		$valid->set_rules('nama','Nama');
			
		$valid->set_rules('nik','NIK','required|is_unique[tb_suruh.nik]',
            array(  'required'    => '%s harus diisi',
        ...
        if($valid->run()==FALSE)    {
            ...
            $data = array(  'title'     => 'Tambah Data suruh',
                            'isi'       => 'kecamatan/suruh/tambah'
            );
            $this->load->view('layout/wrapper', $data, FALSE);
        ///Masuk Database
        }else{
            ...
            $this->suruh_model->tambah($data);
            $this->session->set_flashdata('sukses', 'Data telah ditambah');
            redirect(base_url('kecamatan/suruh'),'refresh');
        }
    }
	...
	public function edit($id)
	{
        ...
        $suruh = $this->suruh_model->detail($id);
        ...
        if($valid->run()==FALSE)    {
            ...
            $data = array(  'title'              => 'Edit Data suruh (no berkas = '.$suruh->no_berkas.' )',
                            'suruh'      => $suruh,
                            'isi'                => 'kecamatan/suruh/edit'
            );
            $this->load->view('layout/wrapper', $data, FALSE);
        ///Masuk Database
        }else{
	...
	public function export_excel()
	{
        ...
        $data = array(  'title'             => 'Daftar Nominatif PTSL Kecamatan suruh',
                        'suruh'     => $suruh
        );
		$this->load->view('kecamatan/suruh/laporan_excel', $data, FALSE);
	}
}
